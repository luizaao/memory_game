...
$(document).ready(function () {
  $(".card").click(function () {
    ...
    if (cardsOpen.length <= 1) {
      ...
      $(this).replaceWith(
        `<div card open${cardsOpen.length
        } class="virado" id="${CardId}" style="background-color:${deckId[CardId - 1].color
        }"><img src="${deckId[CardId - 1].imagem
      ...
      if (cardsOpen[0] == cardsOpen[1]) {
        console.log("entrou");
        $(".virado").replaceWith(
          `<div nada class="no-card"  ><img src="src/logo.png" alt="card" width="100px" height="100px"></div>`
        );
        cardsOpen = cardsNone;
